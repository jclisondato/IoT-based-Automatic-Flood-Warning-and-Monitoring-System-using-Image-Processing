...
def imageprocessing():
    global stop_countdownthread, timecurrent
    ...
    while True:
        ...
        water_level = 100  # Adjust this value as needed
        for contour in contours:
            ...
            if area > water_level:
                ...
                if len(colorcheck) == 5: #Odd number only
                    if frequentcolors() == watercolor[0]:
                        ...
                        timecurrent = map_string_to_value(frequentcolors())
                        print(f"timecurrent value: {timecurrent}")
...
def sendPhotoStatus(prevColor,currentColor):
    ...
    if map_string_to_value(prevColor) > map_string_to_value(currentColor):
        ...
        caption = (f'Water Level is rising, image status interval change to {map_string_to_value(currentColor)}')
        ...
        caption = (f'Water Level is droping, image status interval change to {map_string_to_value(currentColor)}')
        image_path = 'picture.jpg'
        url = f'https://api.telegram.org/bot{api_token}/sendPhoto'
        files = {'photo': open(image_path, 'rb')}
        data = {'chat_id': chat_id, 'caption': caption}
...
def countdown(seconds):
    ...
    for i in range(seconds, 0, -1):
        ...
        data = {'chat_id': chat_id, 'caption': caption}
        requests.post(url, files=files, data=data)
        stop_countdown_thread(map_string_to_value(frequentcolors()))
def start_countdown(duration):
    global stop_countdown, timecurrent, timeprev
    stop_countdown = False
